...
By considering yearly blocks, we face nonstationary concerns for the trend which could probably be imputed to Global Warming. 
...
\begin{enumerate}
	...
	\item \textbf{Expected number of events in a period of \boldsymbol{$m$} years is exactly 1 :} we define  
...
\begin{figure}[!htb]
	\begin{center}
		\resizebox{12cm}{5cm}{ \fbox{
				...
				\foreach \m [count=\y] in {1}
				\node [circle,fill=cyan!50,minimum size=.7cm ] (output-\m) at (4,1.9-\y) {};
				
				\foreach \m [count=\y] in {2}
				\node [circle,fill=cyan!50,minimum size=.7cm ] (output-\m) at (4,1.15-\y) {};
				
				\foreach \m [count=\y] in {3}
				\node [circle,fill=cyan!50,minimum size=.7cm ] (output-\m) at (4,0.45-\y) {};
				...
				%%%%%
				\node [below] at (output-1.north) {$o_1$};
				\node [below] at (output-2.north) {$o_2$};
				\node [below] at (output-3.north) {$o_3$};
				%%%
		...
		\caption{General framework of the fully-connected nonstationary GEV-CDN based on \citet{cannon_flexible_2010}. The input layer will be the time itself in our application, i.e. $x_i(t)=t,  \ \forall i =1,\ldots,I$ but it can be other covariates. The hidden layer represent additional complexity incorporated in the model and the output layer represent the three GEV parameters. \textbf{\textcolor{red}{(1)}} and \textbf{\textcolor{blue}{(2)}} represent the functional relationships (\ref{eq:nnlay1})-(\ref{eq:nnlay2}) between layers. }\label{NN}
	\end{center}
\end{figure}
