...
\hyperref[sec::1.1]{Section \textbf{1.1}} to introduce the emergence of this theory, we will get into the leading theorem of EVT in \hyperref[sec:extrtypethm]{Section \textbf{\ref{sec:extrtypethm}}}. After that, we will present some mathematical applications (\hyperref[sec::appconcrete]{Section \textbf{\ref{sec::appconcrete}}}) and conditions (\hyperref[sec:mda]{Section \textbf{\ref{sec:mda}}}) of this theorem in order to visualize the implications of the extremal theorem and the characterizations of the underlying distributions. Then, \hyperref[rlgev]{Section \textbf{\ref{rlgev}}} will introduce key concepts that help for inference in EVT which will be presented in a general (and frequentist) way in  \hyperref[sec::gevinfernce]{Section \textbf{\ref{sec::gevinfernce}}}. Finally, \hyperref[sec:diag]{Section \textbf{\ref{sec:diag}}} is devoted to list some tools to assess the accuracy of the fitted model.

This chapter is mostly based on \citet[chap.3]{coles_introduction_2001}, \citet[chap.2]{beirlant_statistics_2006} and \citet[chap.1-4]{reiss_statistical_2007}, together with relevant articles.
...
This chapter will focus on another major approach of EV models by modeling only the excess over certain a threshold. This approach is very popular in practice as it can handle all the extremes with more flexibility and not only the maximum of one block. From this fact, lots of techniques have emerged and we will quickly navigate the main ideas.

In \hyperref[sec::2.1]{Section \textbf{\ref{sec::2.1}}}, we will intuitively introduce the approach which will help us 
to formally characterize the resulting distribution of interest in \hyperref[sec:charac_gpd]{Section \textbf{\ref{sec:charac_gpd}}}. Then, \hyperref[sec:rl_gpd]{Section \textbf{\ref{sec:rl_gpd}}} will present the concept of return levels applied in this of context. Whereas meteorologists usually suggest thresholds of $25^{\circ} c$ or $30^{\circ} c$ for maximum temperatures, we will assess the threshold choice in a statistical sense in \hyperref[sec:thresh_selec]{Section \textbf{\ref{sec:thresh_selec}}} by reviewing available methods, but before that we will briefly discuss the parameter estimation in \hyperref[sec::potinference]{Section \textbf{\ref{sec::potinfernce}}}, especially for the EVI. 
...
\citet[chap.5]{embrechts_modelling_2011},
 together with relevant articles.
...
This chapter is mostly based on \citet[chap.5-6]{coles_introduction_2001}, \citet[chap.10]{beirlant_statistics_2006} and \citet[chap.7]{reiss_statistical_2007}, together with relevant articles.
...
This chapter is mostly based on \citet[sec. 9.1]{coles_introduction_2001}, \citet[chap.11]{beirlant_statistics_2006} and \citet[chap.1-4]{reiss_statistical_2007}, together with relevant articles.
...
After having theoretically defined some useful concepts in Extreme Value Theory, we will now 
We already showed the distribution of these data in Figure \ref{fig:pot_plot} to introduce the POT approach. 
