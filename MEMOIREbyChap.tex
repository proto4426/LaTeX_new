...
Sometimes criticized by the community from the possible introduction of some subjectivity, the Bayesian paradigm is increasingly adopted by the practitioners for its benefits. In our case, it provides interesting tools when carrying out an EV analysis. We will then vastly explore this framework. This chapter only focuses on GEV analysis on annual maxima, but extension to POT or Point Processes is straightforward with tools provided in the text or in the code.
 %to provide the big picture to the reader  

In \hyperref[sec:bayprelim]{Section \textbf{\ref{sec:bayprelim}}} we introduce the Bayesian paradigm and present its characteristics. \hyperref[sec:prior]{Section \textbf{\ref{sec:prior}}} presents an essential component of the Bayesian sphere, the priors that we will describe for GEV applications.
\hyperref[sec:baymcmc]{Section \textbf{\ref{sec:baymcmc}}} discusses several Monte Carlo algorithms that can be used to obtain a sample from the posterior. Then, \hyperref[sec:convbay]{Section \textbf{\ref{sec:convbay}}} presents diagnostics to assess that convergence of the algorithms occurred, in order to check if the posterior's sample is reliable. If this is positive,
\hyperref[sec:bayinf]{Section \textbf{\ref{sec:bayinf}}} presents the Bayesian inferential techniques that can be applied.
...
This chapter is mostly based on \citet[sec. 9.1]{coles_introduction_2001},  \citet[chap.11]{beirlant_statistics_2006}, \cite[chap.13]{dey_extreme_2016} and \citet[chap.3]{ag_extremes_2013},  and other relevant articles.
...
Bayesian analysis is more and more sused everywhere. For example,  \hyperref[chap:introana]{Chapter \textbf{\ref{chap:introana}}} used a simulation-based Bayesian approach to compare coverage properties of pointwise and simultaneous confidence intervals, while \hyperref[sec:nnxp]{next Chapter \textbf{\ref{sec:nnxp}}} used priors to constraint the domain of the shape parameter or to penalize weights in a neural architecture.
